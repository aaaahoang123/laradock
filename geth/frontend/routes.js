...
const directoryNode1 = __dirname + "/Node_1";
const directoryNode2 = __dirname + "/Node_2";
var connected = false;
var enode1;
var enode2;
var coinbase1;
...
var killing = false;
var score;
var scoreFile = __dirname + "/score";
var ethhashFolder = __dirname + "/.ethash";
...
module.exports = {
      ...
      if (err) {
        fs.readdir(directoryNode2, function (err, list) {
          if (err) {
            resp.json({
              status: "complete",
              message: "Instance of Ethereum not found at that location.",
            });
            return;
          }
          resp.json({
            status: "error",
            errorDetails:
              "You have already configured Ethereum. Please choose options below.",
          });
          return;
        });
      } else {
    ...
    exec("pkill geth", (err, stdout, stderr) => {
      exec("rm -rf " + directoryNode1, (err, stdout, stderr) => {
        if (err) {
          ...
          resp.json({
            status: "error",
            errorDetails: "Unable to delete both the directories",
          });
          return;
        }
        exec("rm -rf " + directoryNode2, (err, stdout, stderr) => {
          ...
          exec("rm -rf " + ethhashFolder, (err, stdout, stderr) => {
            if (err) {
              resp.json({
                status: "error",
                errorDetails: "Unable to delete the ethhash",
              });
              return;
            }
            resp.json({
              status: "complete",
              message: "Deleted everything successfully.",
            });
          });
        });
      });
    ...
    if (type == ":account") {
      ...
      exec("mkdir " + directoryNode1, (err, stdout, stderr) => {
        if (err) {
          resp.json({
            status: "error",
            errorDetails: "You have already created an account!",
          });
          return;
        }
        fs.writeFile(
          directoryNode1 + "/password.txt",
          req.body.password1,
          function (err) {
            if (err) {
              resp.json({
                status: "error",
                errorDetails: "Unable to create a new account right now!",
              });
              return;
            }
            exec(
              "geth account new --password " +
                directoryNode1 +
                "/password.txt --datadir " +
                directoryNode1,
              (err, stdout, stderr) => {
                if (err) {
                  resp.json({
                    status: "error",
                    errorDetails: "Error Creating your Account right now",
                  });
                  return;
                }
                accountAddress1 = stdout.match(/0x.*/gm)[0];
                exec(
                  "rm " + directoryNode1 + "/password.txt",
                  (err, stdout, stderr) => {
                    if (err) {
                    ...
                    function (err) {
                      if (err) {
                        resp.json({
                          status: "error",
                          errorDetails:
                            "Unable to create a new account right now!",
                        });
                        return;
                      }
                      exec(
                        ...
                        (err, stdout, stderr) => {
                          if (err) {
                            resp.json({
                              status: "error",
                              errorDetails:
                                "Error Creating your Account right now",
                            });
                            return;
                          }
                          accountAddress2 = stdout.match(/0x.*/gm)[0];
                          score.stage1 = 10;
                          updateScoreToFile();
                          resp.json({
                            status: "complete",
                          ...
                          exec(
                            "rm " + directoryNode2 + "/password.txt",
                            (err, stdout, stderr) => {
                              if (err) {
                                console.log("Error removing password file!");
                              }
      ...
      fs.writeFile(
        directoryNode1 + "/customGenesis.json",
        req.body.genesisData,
        function (err) {
          if (err) {
            console.error(err);
            resp.json({
              status: "error",
              errorDetails: "Unable to create the Genesis file.",
            });
            return;
          }
          fs.writeFile(
            directoryNode2 + "/customGenesis.json",
            req.body.genesisData,
            function (err) {
              if (err) {
                console.error(err);
              ...
              resp.json({
                status: "complete",
                message:
                  "Created the Genesis Files with the name customGenesis.json in both nodes.",
              });
            }
          );
        }
        ...
        (err, stdout, stderr) => {
          if (err) {
            ...
            resp.json({
              status: "error",
              errorDetails:
                "Unable to initialize Ethereum. Invalid Genesis file found.",
            });
            return;
          }
          exec(
            ...
            (err, stdout, stderr) => {
              ...
              resp.json({
                status: "complete",
                message:
                  "Initialized both Ethereum nodes. You may start them now.",
              });
            }
          );
        }
      ...
      fs.readdir(directoryNode1, function (err, list) {
        if (err) {
          resp.json({
            status: "error",
            errorDetails: "Ethereum has not been configured right now.",
          });
          return;
        }
        exec(
          [
            ...
            "--port 30301",
            "--http.port 8544",
            "--http.api='txpool,db,eth,net,web3,personal,admin,miner' ",
            "--authrpc.port 8551",
      ...
      setTimeout(function () {
        try {
          ...
          resp.json({
            status: "error",
            errorDetails:
              "Unable to connect to Ethereum. Please go back and start it.",
          });
          return;
        }
        if (web3.isConnected() && web3Node2.isConnected()) {
          ...
          resp.json({
            status: "complete",
            message: "Connected with Ethereum node",
            enode1: enode1,
            coinbase1: coinbase1,
            enode2: enode2,
            coinbase2: coinbase2,
          });
        } else resp.json({ status: "error", errorDetails: "Unable to connect to Ethereum. Please go back and start it." });
      }, 1000);
      connected = true;
    } else {
      if (web3.isConnected() && web3Node2.isConnected()) {
  ...
  ethereum: function (req, resp) {
    ...
    if (type == ":addPeer") {
      ...
      try {
        ...
        if (index != 1) {
          resp.json({
            status: "complete",
            addStatus:
              "Invalid enode provided. Please check enode and try again.",
          });
          return;
        }
      }
      ...
      resp.json({
        status: "complete",
        addStatus:
          "Enode added. You can check the connectivity using peer count or peers.",
      });
    } else if (type == ":peerCount") {
      var node = req.body.node;
      var count;
      if (node == 1) count = web3.net.peerCount;
      else if (node == 2) count = web3Node2.net.peerCount;
      score.stage5 = 10;
      updateScoreToFile();
      ...
      var node = req.body.node;
      var peers;
      if (node == 1) peers = web3.admin.peers;
      else if (node == 2) peers = web3Node2.admin.peers;
      score.stage5 = 10;
      updateScoreToFile();
      if (peers.length == 0) {
        ...
        resp.json({
          status: "complete",
          peers: JSON.stringify(peers, null, 2),
        });
      }
      ...
      if (node == 1) {
        try {
          ...
          resp.json({
            status: "complete",
            unlock: "Incorrect Details Entered",
          });
          return;
        }
      } else if (node == 2) {
      ...
      if (node == 1) {
        ...
        if (accounts.length >= 5) {
          resp.json({
            status: "complete",
            message: "You have already created total 5 accounts on this node.",
          });
          return;
        }
        // for(var k=0; k<4; k++){
        ...
        try {
          ...
          if (index != -1) {
            resp.json({
              status: "complete",
              transactionStatus:
                "Your account is locked. Can not initiate transaction.",
            });
            return;
          }
          ...
          if (index != -1) {
            resp.json({
              status: "complete",
              transactionStatus:
                "Your account has insufficient funds for gas * price + amount that you want to send.",
            });
            return;
          }
          resp.json({
            status: "complete",
            transactionStatus: "An unknown error occured.",
          });
          return;
        }
      } else if (node == 2) {
      ...
      resp.json({
        status: "complete",
        transactionStatus: "Transaction successfully submitted.",
      });
  ...
  submitScore: function (req, resp) {
    if (score) {
      ...
      resp.json({
        status: "complete",
        hash: count.toString() + hashed(JSON.stringify(score)),
      });
    } else {
      resp.json({
        status: "error",
        hash: "Unable to submit your score right now!",
      });
    }
  },
