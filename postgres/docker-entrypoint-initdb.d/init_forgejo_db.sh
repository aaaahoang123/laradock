...
# EOSQL
if [ "$FORGEJO_POSTGRES_INIT" == 'true' ]; then
	psql -v ON_ERROR_STOP=1 --username "$POSTGRES_USER" --dbname "$POSTGRES_DB" <<-EOSQL
		CREATE USER $FORGEJO_POSTGRES_USER WITH PASSWORD '$FORGEJO_POSTGRES_PASSWORD';
		CREATE DATABASE $FORGEJO_POSTGRES_DB;
		GRANT ALL PRIVILEGES ON DATABASE $FORGEJO_POSTGRES_DB TO $FORGEJO_POSTGRES_USER;
		ALTER ROLE $FORGEJO_POSTGRES_USER CREATEROLE SUPERUSER;
	EOSQL
	echo
fi
